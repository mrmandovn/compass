...
- `header`: `"What's sparking this brainstorm?"`
- `question`: `"Where does this idea come from?"`
- `multiSelect`: false
- `options`:
  - `{label: "User / customer pain point", description: "A frustration or unmet need reported by users"}`
  - `{label: "Specific feedback or complaint", description: "A concrete piece of feedback received"}`
  - `{label: "Business goal to hit", description: "A metric, revenue, or activation target we need to reach"}`
  - `{label: "Something a competitor does well", description: "A feature or UX pattern worth exploring"}`
  - `{label: "Spontaneous idea", description: "No clear external source — just an idea worth exploring"}`
...
- `header`: `"Describe the problem"`
- `question`: `"What's the problem or opportunity? Pick one below or write your own."`
- `multiSelect`: false
...
If PO picks 0 → warn `⚠ At least 1 idea must be picked for deep-dive.` then re-ask.
